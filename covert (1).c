...
#include <stdlib.h>
#include <stdint.h>
#include <sys/types.h> // file defines data types
#include <fcntl.h> //performs the operation specified by command on the file descriptor filedes
#include <unistd.h>  //header defines miscellaneous symbolic constants and types
#ifdef _MSC_VER
...
uint64_t eviction_counts[L1_NUM_SETS] = {0};   //eviction_counts set to 0 and empty array
__attribute__ ((aligned (64))) uint64_t trojan_array[32*4096];  //used to decorate a code declaration in C
...
uint64_t* get_eviction_set_address(uint64_t *base, int set, int way)
{
    ...
    int idx_bits = (((uint64_t)base) >> NUM_OFFSET_BITS) & 0x3f; //0x3f: for each bit of yourNumber in binary system presentation use and method
...
void setup(uint64_t *base, int assoc)
{
    ...
    for (i = 0; i < L1_NUM_SETS; i++) {
        eviction_set_addr = get_eviction_set_address(base, i, 0);    //eviction for getting eviction address easily
        for (j = 1; j < assoc; j++) {
...
void trojan(char byte)
{
    ...
    if (byte == 10 || byte == 13) { // encode a new line
     ...
     while(eviction_set_addr != NULL) { // traverse the eviction i.e. memory that is emptied set corresponding to the byte passed to trojan
        eviction_set_addr = (uint64_t *)*eviction_set_addr;
        CPUID();
     }
